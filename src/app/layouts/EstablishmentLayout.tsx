...
import { LayoutDashboard, Users, FileSpreadsheet, HelpCircle, LogOut, GraduationCap, Menu, X, BookOpen, ChevronLeft, ChevronRight } from "lucide-react";
...
export function EstablishmentLayout() {
  ...
  const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(() => {
    const saved = localStorage.getItem('est_sidebar_collapsed');
    return saved === 'true';
  });
  ...
  const toggleSidebar = () => {
    ...
    localStorage.setItem('est_sidebar_collapsed', String(newState));
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col md:flex-row font-sans antialiased selection:bg-emerald-500 selection:text-slate-950 overflow-hidden">
      ...
      <div className="md:hidden flex items-center justify-between p-4 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="font-bold text-sm truncate max-w-[200px] leading-tight">{user?.establishmentName}</span>
            <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider">{user?.establishmentCode}</span>
          </div>
        </div>
      ...
      >
        <div className="p-4 md:p-6 flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 h-20 md:h-auto">
          <div className="bg-blue-600 p-2.5 rounded-xl text-white shadow-lg shadow-blue-900/20 flex-shrink-0">
          ...
          <div className={clsx("min-w-0 flex-1 transition-opacity duration-300", isSidebarCollapsed ? "md:opacity-0 md:hidden" : "opacity-100")}>
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider mt-0.5 truncate">
        ...
        <div className="p-3 flex-1 overflow-y-auto overflow-x-hidden flex flex-col gap-1">
          <nav className="space-y-1">
            {navItems.map((item) => (
              ...
              >
                <item.icon className={clsx("w-5 h-5 flex-shrink-0", isSidebarCollapsed ? "" : "")} />
                <span className={clsx(
