...
export function AdminAccessManagement() {
  ...
  return (
    <div className="max-w-[1700px] mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-100">Gestion des accès</h1>
        <p className="text-slate-400">Contrôle des identifiants et des permissions des établissements.</p>
      </div>
              ...
              {filtered.map((est) => (
                <tr key={est.code} className="hover:bg-slate-800/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-bold text-slate-200">{est.label}</div>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium rounded-lg transition-colors border border-slate-700">
