...
export function AdminDashboard() {
  ...
  return (
    <div className="max-w-[1700px] mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-100">Tableau de bord administrateur</h1>
        <p className="text-slate-400">Vue d'ensemble de la collecte DDESTFP</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="font-medium text-slate-300">Établissements</h2>
          </div>
          <div className="flex items-baseline gap-2">
            <p className="text-3xl font-bold text-slate-100">{ESTABLISHMENTS.length}</p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="font-medium text-slate-300">Enseignants</h2>
          </div>
          <p className="text-3xl font-bold text-slate-100">{stats.totalTeachers}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-slate-100">Établissements récents</h2>
            <Link to="/admin/etablissements" className="text-sm text-emerald-400 hover:text-emerald-300">Voir tout</Link>
          </div>
          <div className="space-y-4">
            {ESTABLISHMENTS.map(est => (
              <div key={est.code} className="flex items-center justify-between p-4 bg-slate-950/50 rounded-xl border border-slate-800">
                <div>
                  <h3 className="font-bold text-slate-100">{est.label}</h3>
                  <p className="text-xs text-slate-400 font-mono">{est.code}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-medium text-slate-300">{est.teacherCount} profs</p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          <h2 className="text-lg font-bold text-slate-100 mb-6">État de la collecte globale</h2>
          <div className="space-y-6">
            <div>
              <div className="flex justify-between text-sm mb-2">
                <span className="text-emerald-400 font-medium">Complètes ({stats.completed})</span>
                <span className="text-slate-400">{stats.totalTeachers > 0 ? Math.round((stats.completed / stats.totalTeachers) * 100) : 0}%</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="text-slate-400">{stats.totalTeachers > 0 ? Math.round((stats.started / stats.totalTeachers) * 100) : 0}%</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2">
            ...
            <div>
              <div className="flex justify-between text-sm mb-2">
                <span className="text-slate-400 font-medium">Non commencées ({stats.notStarted})</span>
                <span className="text-slate-400">{stats.totalTeachers > 0 ? Math.round((stats.notStarted / stats.totalTeachers) * 100) : 0}%</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-2">
            ...
            <div className="pt-6 border-t border-slate-800">
              <Link to="/admin/collectes" className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
