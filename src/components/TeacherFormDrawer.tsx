...
export const TeacherFormDrawer: React.FC<TeacherFormDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex justify-end transition-opacity">
      <div className="w-full max-w-3xl bg-slate-900 border-l border-slate-700 h-full flex flex-col shadow-2xl overflow-hidden">
        {/* Drawer Header */}
        <div className="px-6 py-4 border-b border-slate-800 bg-slate-800/90 flex items-center justify-between shrink-0">
          <div>
            ...
            <h2 className="text-lg font-bold text-slate-100">{teacher.sourceName}</h2>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-1.5 bg-slate-700 hover:bg-slate-600 disabled:opacity-30 disabled:cursor-not-allowed text-slate-200 rounded transition-colors"
            ...
            <button
              onClick={onClose}
              className="p-1.5 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded transition-colors ml-2"
              title="Fermer"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-slate-200">
          ...
          <div className="bg-slate-800/80 border border-slate-700 rounded-xl p-5 space-y-4">
            <h3 className="text-sm font-bold text-purple-300 uppercase tracking-wider flex items-center gap-2 border-b border-slate-700 pb-2">
              <div className="w-2.5 h-2.5 rounded-full bg-purple-500" />
              Carte 1 — Identification
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div>
                <label className="block text-slate-400 font-medium mb-1">Nom et Prénoms (Source)</label>
                <input
                  type="text"
                  readOnly
                  value={teacher.sourceName}
                  className="w-full bg-slate-900 border border-slate-700 text-slate-300 rounded px-3 py-2 cursor-not-allowed"
                />
              </div>
              <div>
                <label className="block text-slate-400 font-medium mb-1">Métier / Spécialité</label>
                <select
                  value={record.specialty || ''}
                  onChange={(e) => handleSpecialtyChange(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-700 text-slate-100 rounded px-3 py-2 focus:border-emerald-500"
                >
          ...
          <div className="bg-slate-800/80 border border-slate-700 rounded-xl p-5 space-y-4">
            <h3 className="text-sm font-bold text-emerald-300 uppercase tracking-wider flex items-center gap-2 border-b border-slate-700 pb-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 text-xs">
              {[
                ...
                <div key={day.name}>
                  <label className="block text-slate-400 font-medium mb-1 text-center">{day.label}</label>
                  <input
                    type="number"
                    min={0}
                    max={12}
                    ...
                    className="w-full text-center bg-slate-900 border border-slate-700 text-slate-100 font-mono rounded py-2 focus:border-emerald-500"
                  />
                </div>
              ))}
            </div>

            <div className="grid grid-cols-2 gap-4 bg-slate-900/60 p-3 rounded-lg border border-slate-700/80 text-xs">
              <div>
                <span className="text-slate-400 block">Total Hebdomadaire (Col K) :</span>
                <span className="text-lg font-bold text-emerald-400 font-mono">{record.weeklyTotal}</span>
              </div>
              <div>
                <span className="text-slate-400 block">Total sur 7 Mois (Col L) :</span>
          ...
          <div className="bg-slate-800/80 border border-slate-700 rounded-xl p-5 space-y-4">
            <h3 className="text-sm font-bold text-sky-300 uppercase tracking-wider flex items-center gap-2 border-b border-slate-700 pb-2">
            ...
            <div className="space-y-4 text-xs">
              {[
                ...
                return (
                  <div key={key} className="bg-slate-900/60 p-3 rounded-lg border border-slate-700/60 space-y-2">
                    <p className="font-semibold text-slate-300">{title}</p>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <div>
                        <label className="text-slate-400 block mb-1">Absences</label>
                        <input
                          type="number"
                          min={0}
                          value={metrics.absences ?? ''}
                          onChange={(e) => handleNumberChange(`${key}.absences`, e.target.value)}
                          className="w-full bg-slate-950 border border-slate-700 text-slate-100 font-mono rounded px-2 py-1"
                        />
                      </div>
                      <div>
                        <label className="text-slate-400 block mb-1">Retards</label>
                        <input
                          type="number"
                          min={0}
                      ...
                      <div>
                        <label className="text-slate-400 block mb-1">Taux Abs %</label>
                        <div className="bg-slate-950 border border-slate-800 text-slate-300 font-mono rounded px-2 py-1 text-center font-bold">
                          {formatPercentage(metrics.absenceRate)}
                        </div>
                      </div>
                      <div>
                        <label className="text-slate-400 block mb-1">Taux Ret %</label>
                        <div className="bg-slate-950 border border-slate-800 text-slate-300 font-mono rounded px-2 py-1 text-center font-bold">
              ...
              <div className="pt-2">
                <label className="text-slate-400 block mb-1 font-medium">Champ Libre « Aux Sur » (Col AO)</label>
                <input
                  type="text"
                  value={record.auxSur || ''}
                  onChange={(e) => handleTextChange('auxSur', e.target.value)}
                  placeholder="Texte ou valeur facultative"
                  className="w-full bg-slate-900 border border-slate-700 text-slate-100 rounded px-3 py-2"
                />
              </div>
            </div>
          </div>
          ...
          {[0, 1, 2].map((classIdx) => {
            ...
            return (
              <div key={`card_class_${classIdx}`} className="bg-slate-800/80 border border-slate-700 rounded-xl p-5 space-y-4">
                <h3 className="text-sm font-bold text-amber-300 uppercase tracking-wider flex items-center gap-2 border-b border-slate-700 pb-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  <div>
                    <label className="block text-slate-400 font-medium mb-1">Libellé de la Classe</label>
                    <input
                      type="text"
                      maxLength={30}
                      value={cls.className || ''}
                  ...
                  <div>
                    <label className="block text-slate-400 font-medium mb-1">Discipline</label>
                    <select
                      ...
                      className="w-full bg-slate-900 border border-slate-700 text-slate-100 rounded px-3 py-2 disabled:opacity-50"
                    >
                      <option value="">
                        {!record.specialty
                          ? 'Spécialité requise'
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                  <div>
                    <label className="block text-slate-400 font-medium mb-1">Effectif Composé</label>
                    <input
                      type="number"
                      min={1}
                      value={cls.composedCount ?? ''}
                      onChange={(e) => handleNumberChange(`classResults.${classIdx}.composedCount`, e.target.value)}
                      className="w-full bg-slate-900 border border-slate-700 text-slate-100 font-mono rounded px-3 py-2"
                    />
                  </div>
                  <div>
                    <label className="block text-slate-400 font-medium mb-1">Moyennes ≥ 10</label>
                    <input
                      type="number"
                      min={0}
                  ...
                  <div>
                    <label className="block text-slate-400 font-medium mb-1">Notes Examen ≥ 10</label>
                    <input
                      type="number"
                      min={0}
                ...
                <div className="grid grid-cols-3 gap-3 bg-slate-900/60 p-3 rounded-lg border border-slate-700/60 text-xs">
                  <div>
                    <label className="block text-slate-400 mb-1">Taux Exécution % (Manuel)</label>
                    <input
                      type="number"
                      step="0.1"
                      min={0}
                      ...
                      className={`w-full bg-slate-950 border font-mono rounded px-2 py-1 text-center ${
                  ...
                  <div>
                    <span className="block text-slate-400 mb-1">Taux Moyen (Calculé)</span>
                    <div className="bg-slate-950 border border-slate-800 text-slate-200 font-mono rounded px-2 py-1 text-center font-bold">
                      {formatPercentage(cls.classSuccessRate)}
                    </div>
                  </div>
                  <div>
                    <span className="block text-slate-400 mb-1">Taux Exam (Calculé)</span>
                    <div className="bg-slate-950 border border-slate-800 text-slate-200 font-mono rounded px-2 py-1 text-center font-bold">
